...
def get_m3u8_from_selenium(url):
    ...
    driver = webdriver.Chrome('/Users/wangxiaowei/Desktop/PythonCode/Python-Spider/documents/chromedriver',
...
'''
# 先持久化存储未解密前的再解密再重新持久化存储
async def dec_key(name,key):
    """
    解密
    :param name:
    :param key:
    ...
    async with aiofiles.open(name,'rb') as f1,\
        aiofiles.open('/Users/wangxiaowei/Desktop/PythonCode/Python-Spider/documents/video_final/temp_'+name_new,'wb') as f2:
            bs = await f1.read()
            await f2.write(aes.decrypt(bs))
    print('处理完毕')

async def aio_dec(key,path_video):
    tasks = []
    for i in range(1,21):
        task = asyncio.create_task(dec_key(path_video+str(i)+'.ts',key))
        tasks.append(task)
...
def merge_ts(path, name):
    ...
    # 切换工作目录到ts视频文件所在
    # os.chdir('/Users/wangxiaowei/Desktop/PythonCode/Python-Spider/documents/video/')
    os.chdir(path)
    # cat命令合成视频
...
def star_download(url):
    ...
    path_m3u8 = '/Users/wangxiaowei/Desktop/PythonCode/Python-Spider/documents/m3u8文件/'
    path_video = '/Users/wangxiaowei/Desktop/PythonCode/Python-Spider/documents/video/{}/'.format(m3u8_name)
